...
** Statues
   ...
   Una forma simple de modelarlo para un solo agente, es usando ~holds(T, red)~
   para indicar cuando no se puede avanzar, y tener una acción ~action(move)~
...
*** Un robot que reparte café
    ...
    - [1.5] Solucione el problema de repartir café. Considere los siguientes
      ...
      #+begin_src prolog
        ...
        % =======
        % Move around
        action(move([SrcRoom,] DstRoom))
        % Position fluents
        fluent(agentAt(Room))

        % Interact with Doors
        action(open([SrcRoom,] Door))
        action(close([DstRoom,] Door))
        % Door Fluents
        fluent(isOpen(Door))
        ...
        start( agentAt(r1, kitchen1)
             ; agentAt(r2, meetingRoom2)).
